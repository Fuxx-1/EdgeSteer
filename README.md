...
EdgeSteer 是一个跨 macOS、Linux 和 Windows 的本地 Rust DNS steering proxy。它用 JSON 定义从单一入口开始的 resolver 图：规则未命中走 `next`，解析器网络或协议失败走 `fallback`，内置插件可绑定在成功的解析层上，把已确认属于 Cloudflare 的地址改写为当前优选 IP。
...
- 配置文件是严格 JSON，未知字段会被拒绝；`entry`、layer、`next`、`fallback` 和 plugin 引用必须存在，`next + fallback` 不能形成环。
