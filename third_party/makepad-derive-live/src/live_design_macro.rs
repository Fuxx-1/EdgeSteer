...
pub fn live_design_impl(input: TokenStream) -> TokenStream {
    ...
    if let Some(span) = parser.span() {
        ...
        tb.add("        cargo_manifest_path: option_env!(").string("EDGESTEER_LIVE_MANIFEST_PATH").add(").unwrap_or(env!(").string("CARGO_MANIFEST_DIR").add(")).trim_start_matches(").string("\\\\?\\").add(").to_string(),");
